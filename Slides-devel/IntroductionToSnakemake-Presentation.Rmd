...
1. Background - Motivation for workflow managers
2. Snakemake - How to install Snakemake
3. Basics of Snakemake - Hello World, Wildcards, DAG visualisation
4. Advanced Snakemake - Expand function, own scripts, target rules, multi-threading, configuration files, input functions
5. Breakout sessions - Hands-on exercises
...
2. Is my analysis reusable?
3. Can I easily restart a crashed analysis run?
4. How do I share my analysis with others?
5. Where do I get all the required software from?
6. How do I version the software I use?
...
5. It is straight forward to install

## How to install Snakemake?
<div class="my-footer">How to install Snakemake?</div> 

You would need to fulfill the some basic requirements 

1. You would need to have Python >= 3.8 installed

```{bash, eval=FALSE}
$ python --version
``` 

2. Using conda makes it very easy to install Snakemake, so you could check the availablity
...
<p class="warning">
If this throws an error, proceed to 'Install via pip' or install conda.
</p>
...
However, keep in mind that Anaconda is a commercial product, so you should be aware of their licensing terms.
...
In case the conda dependency solver takes too long, try mamba instead.

## Install Snakemake via pip
<div class="my-footer">Install Snakemake via pip</div> 

If you do not want to use conda to install Snakemake, you can also use pip. Here, it is the easiest to create a virtual environment first

```{bash, eval=FALSE}
cd /path/to/where/you/want/to/create/the/env
mkdir myvenv
python -m venv myvenv/
```

Then activate that environment and install snakemake as well as some dependencies into it

```{bash, eval=FALSE}
source myvenv/bin/activate
pip3 install snakemake==9.14.5
pip3 install snakemake-executor-plugin-slurm snakemake-storage-plugin-http
pip3 install snakemake-executor-plugin-cluster-generic
pip3 install pandas numpy matplotlib jinja2 pygments networkx graphviz pandoc
```

If you want to use Snakemake, you can later just activate the environment again

```{bash, eval=FALSE}
source myvenv/bin/activate
```

and also, you can deactivate it in a similar way
...
## Snakemake on CSC/Puhti or other maintained HPC systems
